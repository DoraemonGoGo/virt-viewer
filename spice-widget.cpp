...
static void callbackInvalidate(SpiceDisplayPrivate *d, gint x, gint y, gint w, gint h)
{
    ...
    SpiceWindow::getSpice()->updateImage(img, x, y, w, h);
}

static void callbackSettingsChanged(gint instance, gint width, gint height, gint bpp)
{
    SpiceWindow::getSpice()->settingsChanged(width, height, bpp);
}
...
{
    SpiceWindow::getSpice()->showCursor(true);
}
...
{
    SpiceWindow::getSpice()->showCursor(false);
}
...
static void channel_new(SpiceSession *s, SpiceChannel *channel, gpointer data)
{
    ...
    if (SPICE_IS_INPUTS_CHANNEL(channel)) {
        ...
        spice_channel_connect(channel);
        guint32 modifiers;
        modifiers = SpiceWindow::getSpice()->getKeyboardLockModifiers();
